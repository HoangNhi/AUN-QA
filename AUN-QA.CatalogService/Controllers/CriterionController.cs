...
using AUN_QA.CatalogService.Services.CoreFeature.Criterion;
...
namespace AUN_QA.CatalogService.Controllers
{
    ...
    [ApiController]
    public class CriterionController : BaseController<CriterionController>
    {
        ...
        public async Task<IActionResult> GetList(GetListPagingRequest request)
        {
            ...
            return Ok(new BaseResponse<GetListPagingResponse<ModelCriterion>> { Data = result, Success = true });
        }
        ...
        public async Task<IActionResult> GetById([FromQuery] GetByIdRequest request)
        {
            ...
            return Ok(new BaseResponse<ModelCriterion> { Data = result, Success = true });
        }

        [HttpPost("insert")]
        [AttributePermission(Action = ActionType.ADD)]
        public async Task<IActionResult> Insert([FromBody] CriterionRequest request)
        {
            if (!ModelState.IsValid)
                return Ok(new BaseResponse(false, 400, CommonFunc.GetModelStateAPI(ModelState)));
        ...
        public async Task<IActionResult> Update(CriterionRequest request)
        {
            if (!ModelState.IsValid)
                return Ok(new BaseResponse(false, 400, CommonFunc.GetModelStateAPI(ModelState)));
