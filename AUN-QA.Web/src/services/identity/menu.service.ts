...
export const menuService = {
  getList: async (
    ...
    return api.post<GetListPagingResponse<Menu>>(
      API_ENDPOINTS.Identity.Menu.GET_LIST,
      request
    );
  },

  getById: async (id: string): Promise<ApiResponse<Menu>> => {
    return api.get<Menu>(API_ENDPOINTS.Identity.Menu.GET_BY_ID, {
      params: { id },
    });
  },

  insert: async (data: Menu): Promise<ApiResponse<Menu>> => {
    return api.post<Menu>(API_ENDPOINTS.Identity.Menu.INSERT, data);
  },

  update: async (data: Menu): Promise<ApiResponse<Menu>> => {
    return api.put<Menu>(API_ENDPOINTS.Identity.Menu.UPDATE, data);
  },

  deleteList: async (ids: string[]): Promise<ApiResponse<Menu[]>> => {
    return api.delete<Menu[]>(API_ENDPOINTS.Identity.Menu.DELETE_LIST, {
      data: { ids },
    });
  },
};
