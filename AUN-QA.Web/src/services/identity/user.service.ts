...
export const userService = {
  getList: async (request: GetListPagingRequest): Promise<ApiResponse<GetListPagingResponse<User>>> => {
    return api.post<GetListPagingResponse<User>>(API_ENDPOINTS.Identity.User.GET_LIST, request);
  },

  getById: async (id: string): Promise<ApiResponse<User>> => {
    return api.get<User>(API_ENDPOINTS.Identity.User.GET_BY_ID, { params: { id } });
  },

  insert: async (user: User): Promise<ApiResponse<User>> => {
    return api.post<User>(API_ENDPOINTS.Identity.User.INSERT, user);
  },

  update: async (user: User): Promise<ApiResponse<User>> => {
    return api.put<User>(API_ENDPOINTS.Identity.User.UPDATE, user);
  },

  deleteList: async (ids: string[]): Promise<ApiResponse<User[]>> => {
    return api.delete<User[]>(API_ENDPOINTS.Identity.User.DELETE_LIST, { data: { ids } });
  },
};
